---
title: "KG_Basic_dataWhale学习笔记（四）"
tag: "KnowledgeGraph"
date: 2021-01-14
---

# KG_Basic_dataWhale学习笔记（四）

## Task04---用户输入以及知识库的查询操作

## 一、任务简介

本部分任务主要是**将用户输入问答系统的自然语言转化成知识库的查询语句**。解释操作的过程。
...
* 问答系统从实现方式划分：
  * **基于流水线**（pipeline）实现：如下图 1 所示，基于流水线实现的问答系统有**四大核心模块**，分别由**自然语言理解（NLU）、对话状态跟踪器（DST）、对话策略（DPL）和自然语言生成（NLG）依次串联构成的一条流水线**，各模块可独立设计，模块间协作完成任务。
  * **基于端到端**（end-to-end）实现：基于端到端实现的问答系统，**主要是结合深度学习技术**，通过海量数据训练，挖掘出从用户自然语言输入到系统自然语言输出的整体映射关系，而忽略中间过程的一种方法。但就目前工业界整体应用而言，工业界的问答系统**目前大多采用的还是基于流水线实现的方式。**

![流水线的操作](https://gitee.com/magicye/blogimage/raw/master/img/10798244-16aa357b7be5a646.png)

* 问答系统从答案来源划分：
  * 「知识库问答」。是目前的研究热点。**知识库问答**（knowledge base question answering, KB-QA）即给定自然语言问题，通过对问题进行语义理解和解析，进而利**用知识库进行查询、推理得出答案**。如下图所示：
  * 「常问问题问答」；
  * 「新闻问答」；
  * 「网际网路问答」；

![知识库查询](https://gitee.com/magicye/blogimage/raw/master/img/10798244-afb41aa23fee13c7.png)

### 2.2 Query理解

#### 2.2.1  Query理解介绍（询问理解）
...
由于本任务后面代码**主要涉及意图识别和槽位填充**，因此这里仅对这两部分内容做介绍：
...
![意图识别](https://gitee.com/magicye/blogimage/raw/master/img/10798244-467c5be884303091.png)

#### 2.2.3 槽值填充

- 介绍：槽值填充就是**根据我们既定的一些结构化字段，将用户输入的信息中与其对应的部分提取出来。**因此，槽值填充经常被建模为序列标注的任务。
...
![槽值填充](https://gitee.com/magicye/blogimage/raw/master/img/10798244-25aa5d0560dfee1a.jpg)

- **序列标注的任务常用的模型**有：
- 【注：这部分内容，第二期知识图谱组队将进行介绍】
  - 词典匹配；
  - **BiLSTM + CRF**；
  - **IDCNN**
  - BERT等。
...
![基于知识图谱的问答系统框架](https://gitee.com/magicye/blogimage/raw/master/img/10798244-322785573485895d.png)
...
![实体介绍](https://gitee.com/magicye/blogimage/raw/master/img/10798244-b593dcc06d95bb35.png)
...
![意图识别整体举例介绍](https://gitee.com/magicye/blogimage/raw/master/img/10798244-6113c647b881e4aa.png)
